...
function MMI.clean!(model::LaplaceClassification)
    warning = ""
    if model.lambda < 0
...
) where {M1<:LaplaceClassification,M2<:LaplaceClassification}
...
function MMI.clean!(model::LaplaceRegression)
    warning = ""
    if model.lambda < 0
        warning *= "Need `lambda ≥ 0`. Resetting `lambda = 0`. "
        model.lambda = 0
    ...
    if model.likelihood != :regression
        warning *="Need `likelihood ∈ (:regression)`. "*
        "Resetting `likelihood = :classification`. "
